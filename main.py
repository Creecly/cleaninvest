...
# МОДЕЛИ (упрощенные и оптимизированные)
...
class User(db.Model):
    ...
    total_profit = db.Column(db.Column(db.Float, default=0.0)
    investments_count = db.Column(db.Integer, default=0)
    successful_investments = db.Column(db.Integer, 0)
    failed_investments = db.Column(db.Integer, 0)
...
class SupportChat(db.Model):
    ...
    def to_dict(self):
        ...
        last_message = messages[-1].to_dict() if messages and len(messages) > 0 else None
...
class ChatMessage(db.Model):
    __tablename__tablename__ = 'chat_message'
    id = db.Column(db.Integer, primary_key=True)
...
def send_welcome_email(user, password):
    """Send welcome email with proper Flask context"""
...
def send_stock_growth_email(user, company, growth_percentage):
    """Send stock growth email with proper Flask context"""
...
# АВспомогательные функции
def send_email_async(user, subject, recipients, html_content):
    """Send email asynchronously with proper Flask context"""
    ...
    def update_profile():
        ...
        if 'phone' in data:
            ...
            if phone and not re.match(
                    r'^[\+]?[(]?[(]?[0-9]{1,4}[)]?[-\s\.]?[(]?[0-9]{1,4}[)]?[-\s\.]?[0-9]{1,9}]$')', phone):
...
def get_chat_messages(chat_id):
    ...
    if chat.user_id != user.id and (not user.is_admin or chat.admin_id != user.id):
        ...
        for message in messages:
            if
        not message.is_system and message.sender_id != user.id:
